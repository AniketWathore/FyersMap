...
╠══════════════════════════════════════════════════════════════╣
║  LEFT  : Scrollable DOM table (all accumulated levels)      ║
║          Bids above MID, asks below MID, sorted by price ↑  ║
║          MID row moves with mid price                       ║
║                                                              ║
...
║          · Zoom/pan → auto-pauses auto-scroll                ║
║          · Press PLAY → resumes auto-scroll                  ║
║          · Chart Y-axis synced to DOM, mid line centre-aligned ║
╚══════════════════════════════════════════════════════════════╝
"""
...
#  CONFIGURATION
# ─────────────────────────────────────────────────────────────
PRICE_LEVELS     = 50        # original file depth (only used as initial guess)
...
# ═════════════════════════════════════════════════════════════
#  ORDERBOOK TRACKER  (accumulates all price levels forever)
# ═════════════════════════════════════════════════════════════
class OrderbookTracker:
    """Returns two sorted lists (ascending price):
         bids_asc: price from low to high (all bids)
         asks_asc: price from low to high (all asks)
    """
    def __init__(self):
    ...
    def update(self, snap: pd.DataFrame, mid: float) -> Dict:
        if snap.empty:
            return {'bids_asc': [], 'asks_asc': [], 'mid': mid}
        ...
        # All bids sorted ascending (lowest → highest)
        bids_asc = sorted(
            [ (lv.price, lv.cumulative_qty, lv.cumulative_orders)
              for lv in self.bid_levels.values() ],
            key=lambda x: x[0]
        )
        # All asks sorted ascending (lowest → highest)
        asks_asc = sorted(
            [ (lv.price, lv.cumulative_qty, lv.cumulative_orders)
              for lv in self.ask_levels.values() ],
            key=lambda x: x[0]
        )
...
# ═════════════════════════════════════════════════════════════
#  DELEGATES  (unchanged, reused by dynamic DOM)
# ═════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════
#  ORDERBOOK WIDGET  —  dynamic rows, keeps all price levels
# ═════════════════════════════════════════════════════════════
class OrderbookWidget(QFrame):
    """
    Dynamic table: rows = all bids (asc) + 1 MID row + all asks (asc).
    Auto‑centres the MID row whenever data changes (unless user scrolls away).
    """
    def __init__(self, parent=None):
        ...
        self._tick_ctr   = 0
        self._centred    = False
        self._mid_idx    = 0          # row index of the MID row
        self._price_map  = []        # (price, side) for every visible row (including MID)
        self._build()
    ...
    def _build(self):
        ...
        self.tbl = QTableWidget(0, 3)          # start with 0 rows
    ...
    def _cell(self, r, c, rt, iv, txt):
        """Guarantee that a QTableWidgetItem exists at (r,c) then set its data."""
    ...
    def refresh(self, ob: Dict, bid: float, ask: float, mid: float):
        ...
        bids_asc = ob.get('bids_asc', [])  # [(price, cum_qty, cum_orders), …]
        asks_asc = ob.get('asks_asc', [])

        n_bids = len(bids_asc)
        n_asks = len(asks_asc)
        total_rows = n_bids + 1 + n_asks
        self._mid_idx = n_bids                # row index of MID (0‑based)

        # build a linear price map for Y‑sync (price of every row, MID = mid)
        self._price_map = [p for p, _, _ in bids_asc] + [mid] + [p for p, _, _ in asks_asc]

        # update max values for intensity bars
        all_q = [q for _, q, _ in bids_asc + asks_asc]
        all_o = [o for _, _, o in bids_asc + asks_asc]
        ...
        # adjust table row count
        self.tbl.setRowCount(total_rows)
        for r in range(total_rows):
            self.tbl.setRowHeight(r, ROW_H)

        # Clear all spans before setting the MID row span (avoid orphaned spans)
        self.tbl.clearSpans()
        self.tbl.setSpan(self._mid_idx, 0, 1, 3)
        ...
        if mi is None:
            mi = QTableWidgetItem()
            self.tbl.setItem(self._mid_idx, 0, mi)
        ...
        # auto‑centre MID row (only if not manually scrolled away, but we just redraw)
        self._centre()

        # stats
        tb = int(sum(q for _, q, _ in bids_asc))
        ta = int(sum(q for _, q, _ in asks_asc))
        self.stat.setText(
            f"  BID Qty: {tb:>10,}  │  ASK Qty: {ta:>10,}  │  LEVELS: {n_bids}B/{n_asks}A"
        )

    def _centre(self):
        """Scroll the table so the MID row is exactly at the viewport centre."""
        if self._mid_idx < 0 or self.tbl.rowCount() == 0:
            return
        row_h = self.tbl.rowHeight(self._mid_idx) or ROW_H
        viewport_h = self.tbl.viewport().height()
        target_scroll = max(0, self._mid_idx * row_h - viewport_h // 2 + row_h // 2)
        self.tbl.verticalScrollBar().setValue(target_scroll)

    def get_visible_price_range(self) -> Tuple[float, float, float]:
        """
        Return (top_price, bottom_price, mid) of the rows currently visible
        in the table's viewport.  Used to align the chart's Y‑axis.
        """
        if not self._price_map:
            return 0.0, 0.0, 0.0

        scroll = self.tbl.verticalScrollBar().value()
        vp_height = self.tbl.viewport().height()
        top_idx = max(0, min(len(self._price_map)-1, scroll // ROW_H))
        bot_idx = max(0, min(len(self._price_map)-1, (scroll + vp_height) // ROW_H))

        top_price = self._price_map[top_idx]
        bot_price = self._price_map[bot_idx]
        mid_price = self._price_map[self._mid_idx] if 0 <= self._mid_idx < len(self._price_map) else 0.0
        return top_price, bot_price, mid_price
...
# ═════════════════════════════════════════════════════════════
#  TRADING CHART  —  synced to DOM, mid line centre‑aligned
# ═════════════════════════════════════════════════════════════
class TradingChart(pg.GraphicsLayoutWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._user_zoomed_x = False
        self._user_zoomed_y = False
        self._build()
    ...
    def on_tick(self, mid: float, tick_num: int, timestamp: int):
        ...
        if not self._user_zoomed_x:
            self._auto_range_x(x, n)

    def _auto_range_x(self, x, n):
        vs = max(0, n - 600)
        self.plt.setXRange(vs, n, padding=0.02)

    def sync_y_to_dom(self, top_price: float, bottom_price: float, mid: float):
        """
        Align the chart's Y‑axis so that the dotted MID line is exactly
        at the vertical centre, and the visible range covers at least the
        DOM's visible prices.
        """
        if self._user_zoomed_y:
            return
        if top_price <= 0 or bottom_price <= 0:
            return
        half_span = max(abs(mid - top_price), abs(bottom_price - mid), 0.5)
        self.plt.setYRange(mid - half_span, mid + half_span, padding=0)

    def _on_manual(self, vb, ranges):
        """Called when user zooms/pans.  Set both X and Y flags."""
        self._user_zoomed_x = True
        self._user_zoomed_y = True
...
class BookmapTerminal(QMainWindow):
    ...
    def _step(self):
        ...
        # Sync chart Y axis to DOM visible price range (mid line centre‑aligned)
        top_p, bot_p, _ = self.dom.get_visible_price_range()
        self.chart.sync_y_to_dom(top_p, bot_p, mid)
