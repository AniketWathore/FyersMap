...
class TradingChart(pg.GraphicsLayoutWidget):
    ...
    X-axis:  latest tick is always pinned near the right edge with a small
             empty right-margin (CHART_RIGHT_PAD fraction of the window).
             Zoom/pan is fully supported; panning disables auto-follow
             until chart reset/restart.

    Y-axis:  driven by the orderbook's visible price window so the dotted
             mid-price line visually lines up with the DOM mid row.

    Badge:   price label sits on the right axis, anchored to the right edge
             of the view — it never drifts off screen.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
    ...
    def _build(self):
        ...
        # horizontal dotted last-price line
        ...
        # ── Price badge — anchored to RIGHT edge of view ──────
        # anchor=(1, 0.5): right-edge of the text box sits at the
        # given (x, y) position.  We position it at (right_edge, mid)
        # so the badge hugs the right y-axis.
        ...
        # vertical "now" dashed line (at last tick)
        ...
        self._mprx = pg.SignalProxy(
            self.plt.scene().sigMouseMoved, rateLimit=30,
            slot=self._on_mouse)

        # Manual range change → stop auto-follow
        self.plt.vb.sigRangeChangedManually.connect(self._on_manual)

    # ── Y-range sync (driven by orderbook visible window) ────
    def sync_y_range(self, low_price: float, high_price: float):
        """
        Set chart Y-range to exactly the orderbook's visible price band.
        This keeps the dotted mid line at the same vertical position as
        the mid row in the DOM panel.
        """
        if not self._user_zoomed and low_price < high_price:
            self.plt.setYRange(low_price, high_price, padding=0)

    # ── TradingView-style X auto-range ───────────────────────
    def _apply_auto_range_x(self, n: int) -> float:
        """
        Keep the LATEST tick pinned near the right edge with a small
        empty margin (CHART_RIGHT_PAD fraction of the visible window).

        Returns the X coordinate of the right edge of the new view so
        the badge can be positioned there.
        """
        if n == 0:
            return 0.0
        latest      = float(n - 1)
        margin_ticks = max(6.0, CHART_VISIBLE * CHART_RIGHT_PAD)
        right_edge   = latest + margin_ticks
        left_edge    = max(0.0, right_edge - CHART_VISIBLE)
        self.plt.setXRange(left_edge, right_edge, padding=0)
        return right_edge
    ...
    def on_tick(self, mid: float, tick_num: int, timestamp: int):
        ...
        if len(self._prices) > CHART_MAX_POINTS:
            ...
            self._taxis._x2s = {
                i: s for i, s in enumerate(self._ts_sec)}
        ...
        # coloured dots (green = up, red = down)
        if n > 1:
        ...
        # horizontal price-level dotted line
        self._lp_line.setValue(mid)

        # "now" vertical marker at the last tick
        self._now_line.setValue(float(n - 1))

        # ── Auto-range X: keep latest at right edge ──────────
        if not self._user_zoomed:
            right_x = self._apply_auto_range_x(n)
        else:
            # user has zoomed/panned — respect their view,
            # but still update badge at their current right edge
            right_x = self.plt.vb.viewRange()[0][1]

        # ── Badge: right-aligned AT the right edge of the view
        # anchor=(1.0, 0.5) means the RIGHT side of the text sits
        # exactly at (right_x, mid), so it hugs the right axis.
        self._badge.setPos(right_x, mid)
        self._badge.setText(f' ₹{mid:.2f} ')

    # ── event handlers ────────────────────────────────────────
    def _on_manual(self, *_):
        """User dragged/zoomed the chart → freeze auto-follow."""
    ...
    def reset(self):
        self._prices.clear()
        self._ts_sec.clear()
        self._tick_nums.clear()
        self._origin_sec  = None
        self._origin_tick = 0
        self._user_zoomed = False
